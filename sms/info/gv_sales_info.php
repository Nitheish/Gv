<?
	$customerArr = array();
	$customerArr[-1] = "Add New Customer";
	$customerArr[0] = "Non Customer";
	$MySelectQry = "select customer_name,customer_phone,customer_id from customer_info where";
	...
	$MySelectQry .= " order by customer_name";
	$MyDbResult =  MyDbQuery($MySelectQry,$_GVParamArray);	
	while($MyRowData = mysqli_fetch_array($MyDbResult, MYSQLI_ASSOC)) 
	{
		$customerArr[GVStringFormat($MyRowData['customer_id'])] = strtoupper(GVStringFormat($MyRowData['customer_name']))." ( ".GVStringFormat($MyRowData['customer_phone'])." )";
	}
